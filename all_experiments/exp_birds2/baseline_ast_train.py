...
if __name__ == "__main__":
    ...
    input_train_data = (input_train_data * 255).astype(np.uint8)
    inputs_train = image_processor(input_train_data, return_tensors="pt")
    
    input_val_data = np.array([val_dataset_encoded[i]['input_values'] for i in range(len(val_dataset_encoded))])
    input_val_data = np.repeat(input_val_data[..., np.newaxis], 3, axis=-1)
    input_val_data = (input_val_data * 255).astype(np.uint8)
    inputs_val = image_processor(input_val_data, return_tensors="pt")
    ...
    model = EfficientNetForImageClassification.from_pretrained(args.model_name, num_labels=num_labels, label2id=label2id, id2label=id2label, ignore_mismatched_sizes=True) 
    ...
    trainer = Trainer(
        ...
        train_dataset=input_train_data,                                         # passing the encoded train set
        eval_dataset=input_val_data,                                            # passing the encoded val set
