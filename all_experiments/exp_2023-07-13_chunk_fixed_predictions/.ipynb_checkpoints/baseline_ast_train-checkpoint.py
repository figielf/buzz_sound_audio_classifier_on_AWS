...
def preprocess_data_for_training(
    ...
    if files_filter:
        print(dataset)
        dataset.filter(lambda x: x['file_name'].startswith(files_filter))
        print(dataset)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--chuncked_predictions", type=bool, default=False)
    ...
    if chuncked_predictions:
        ...
        for file_to_filter in files:
            ...
            test_dataset_encoded = preprocess_data_for_training(dataset_path=test_path, sampling_rate=args.sampling_rate, feature_extractor=feature_extractor,
                                                                fe_batch_size=args.fe_batch_size, dataset_name="test", files_filter=file_to_filter)     

            val_dataset_encoded = preprocess_data_for_training(dataset_path=val_path, sampling_rate=args.sampling_rate, feature_extractor=feature_extractor,
                                                               fe_batch_size=args.fe_batch_size, dataset_name="validation", files_filter=file_to_filter)

            val_dataset_encoded.set_format(type='torch', columns=['input_values'])
            val_dataset_encoded = val_dataset_encoded.map(lambda x: make_chunked_predictions(x['input_values'], model, device, x['label']), batched = True, batch_size=args.eval_batch_size)
            ...
            val_dataset_encoded['logits'] = val_dataset_encoded['logits'].apply(lambda x: x[0])   # extract floats

            val_dataset_encoded.to_csv(f"{args.output_dir}/prediction_val_{file_to_filter}.csv", index = False) # saving the file with validation predictions
            ...
            test_dataset_encoded = test_dataset_encoded.map(lambda x: make_chunked_predictions(x['input_values'], model, device), batched = True, batch_size=args.eval_batch_size)   
            ...
            test_dataset_encoded_df.to_csv(f"{args.output_dir}/prediction_test_{file_to_filter}.csv", index = False)  # saving the file with test predictions
