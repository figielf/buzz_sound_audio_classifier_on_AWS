...
TARGET_FOLDER ='data_copied_by_data_utils_py'
...
def copy_data_to_s3(source_folder, subfolders, data_map_filter, s3_client, bucket_name, bucket_subfolder='data_new'):
    ...
    s3_target_labels_file_path = os.path.join(TARGET_FOLDER, bucket_subfolder, 'labels.json')
    ...
    for subfolder in subfolders:
        ...
        copy_data_subfolder_to_s3(source_folder, subfolder, data_map_filter, s3_client, bucket_name, bucket_subfolder=bucket_subfolder)
        print(f'Copy finished\n')
    

def copy_data_subfolder_to_s3(source_folder, subfolder, data_map_filter, s3_client, bucket_name, bucket_subfolder='data_new'):
    source_folder_full = os.path.join(DATA_FOLDER, source_folder)
    ...
    s3_target_metadata_file_path = os.path.join(TARGET_FOLDER, bucket_subfolder, subfolder, 'metadata.csv')
    ...
    for file_name in files_to_move:
        if file_name.endswith('.wav'):            
            ...
            target_file_path = os.path.join(TARGET_FOLDER, bucket_subfolder, subfolder, file_name)
