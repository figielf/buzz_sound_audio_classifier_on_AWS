...
if __name__ == "__main__":
    ...
    inputs_train = image_processor(x_norm, return_tensors="pt")
    
    input_val_data = np.array([val_dataset_encoded[i]['input_values'] for i in range(len(val_dataset_encoded))])
    input_val_data = abs(input_val_data)
    input_val_data = np.repeat(input_val_data[..., np.newaxis], 3, axis=-1)
    x_val_norm = (input_val_data-np.min(input_val_data))/(np.max(input_val_data)-np.min(input_val_data))
    inputs_val = image_processor(x_val_norm, return_tensors="pt")
    ...
    trainer = Trainer(
        ...
        train_dataset=inputs_train,                                         # passing the encoded train set
        eval_dataset=inputs_val,                                            # passing the encoded val set
