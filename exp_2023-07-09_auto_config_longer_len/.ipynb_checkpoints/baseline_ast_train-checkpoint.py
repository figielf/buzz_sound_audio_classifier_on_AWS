...
def get_feature_extractor(model_name: str, 
                          ...
                          max_length: int=1024) -> ASTFeatureExtractor:
    """
    ...
    if all((train_dataset_mean, train_dataset_std)):
        feature_extractor = ASTFeatureExtractor.from_pretrained(model_name, mean=train_dataset_mean, std=train_dataset_std, num_mel_bins=num_mel_bins, max_length=max_length)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_mel_bins", type=int, default=128)                           # number of bins for mel spectrogram  
    ...
    feature_extractor = get_feature_extractor(args.model_name, args.train_dataset_mean, args.train_dataset_std, args.num_mel_bins, args.max_length)
    ...
    config = ASTConfig.from_pretrained(args.model_name, 
                                   num_labels=num_labels, 
                                   label2id=label2id, 
                                   id2label=id2label,
                                   num_mel_bins = args.num_mel_bins,
                                   max_length=args.max_length)
    model = ASTForAudioClassification.from_pretrained(args.model_name, config = config, ignore_mismatched_sizes=True)        
